...
\label{chap:I-2-lhc}
  ...
  \section{Scientific Motivations}
    ...
    The high energy and luminosity at which the LHC operates are the key factors of its success. Reaching new energy frontiers will enable physicists to discover new hidden sectors of nature at a scale never reached before. However, due to the rarity of these event that lie beyond the standard model, it is import to accumulate a large statistics and thus run the machine at a high luminosity. \\

    The discovery of the Brout-Englert-Higgs boson is one of the main goals of the LHC which has already been fulfilled. However, the caracterisation of the particle still requires a great amount of analysis in order to, for example, quantify the coupling constants to fermions on weak bosons. \\

    Not only physics beyond the standard model is being tested by the LHC. Measuring the free parameters of the theory and comparing them to theoretical and previously obtained values is also an important task which helps validating the models. Reaching new energies and luminosities allow to study rare processes such as the $ B^0_S $ meson decay, $ \bar{b}s \rightarrow \mu + \bar{\mu} $, which was first observed at the LHC.
