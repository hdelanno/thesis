...
\label{chap:I-3-cms}
  ...
  \section{The Muon System}
    ...
    In the barrel, four cylindrical layers of DTs, numbered MB1 to MB4, are spread over a pseudorapidity range of $ | \eta | $ < 1.2. The first three layers each contain eight chambers: four to measure the r-$\phi$ bending angle and four to measure the z direction. The fourth station solely tracks the r-$\phi$ parameter using four chambers. \\
    ...
    \subsection{Drift Tubes}
      ...
      The DT technology is suitable for the barrel region where particle rates stay low and occupancy in the chambers is limited. The building block of DTs are drift cells in which a 2.4 m wire is stretched inside a 21 mm wide cell filled with ArCO$_2$. Four staggered layers of drift cells are assembled together in order to form a Super Layer (SL) which, by design, reduces eliminate deadspace between the cells and improves the timing resolution. SLs vary in size between 1.9 m in MB1 and 4.1 m in MB4. Each chamber contains two SLs that measure the r-$\phi$ bending angle and one SL that provides a measurment in the z direction. MB4, the fourth station, however does not include the SLs in the z direction.
