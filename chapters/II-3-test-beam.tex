...
\label{chap:II-3-test-beam}
  ...
  \section{The Test Beam Setup}
    ...
    During the November 2015 test beam campaign, a superchamber of GE1/1-V detectors has been used to collect data. Each detector was equipped with 12 VFAT2 Hybrids mounted in the central four rows. The remaining slots, not exposed to the beam, have been covered with grounding equipement to reduce noise. Each GEM was mounted with a GEB v2 and a OptoHybrid v2a. A picture of the setup is represented in Figure \ref{fig:II-3-test-geb}. Both OptoHybrids were connected to the same GLIB placed in a microTCA crate along with an AMC13 to deliver the triggers. The GEM detectors were connected to a gas system providing a mixture of Ar/CO$_2$ at 70\% and 30\% respectivly. The high-voltage was delivered for one GEM through a ceramic high-voltage divider and for the other by separate channels connected directly to the GEM foils and planes. The high-voltages are expressed in terms of current flowing through the divider which by suming up the individual resistors yields a value in volts. Table \ref{tab:II-3-test-hv} enumarates the different values of the voltages according to the current flowing through the divider.
